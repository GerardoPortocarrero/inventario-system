...
const AdminUploadPage: FC = () => {
  ...
  useEffect(() => {
    ...
    // Verificar reportes existentes para el estado inicial
    const reportRef = ref(rtdb, 'reportes/volumen/metadata');
    const reportUnsub = onValue(reportRef, (snapshot) => {
      if (snapshot.exists()) {
  ...
  return (
    <div className="admin-layout-container flex-column h-100 overflow-auto custom-scrollbar">
      <div className="admin-section-table d-flex flex-column h-100 w-100 p-2 p-md-3">
        <div className="w-100">
          <Alert variant="warning" className="d-flex align-items-center mb-4 border-0 shadow-sm mx-0" style={{ backgroundColor: 'rgba(244, 0, 9, 0.1)', color: 'var(--theme-text-primary)', borderLeft: '4px solid var(--color-red-primary) !important' }}>
            <FaExclamationTriangle className="me-3 fs-4 flex-shrink-0 text-danger" />
            <div style={{ fontSize: '0.85rem' }}>
              <strong>Atención:</strong> El sistema reemplazará completamente la información y regenerará los reportes operativos.
            </div>
          </Alert>
              ...
              <Col key={type} xs={12} xl={6} className="px-0 px-md-2 m-0">
                <div 
                  ...
                  style={{ 
                    ...
                    borderLeft: `4px solid var(--bs-${type === 'maestro' ? 'danger' : 'primary'})`
                  }}
                >
                  <div className="d-flex align-items-center mb-3">
                    <div className={`p-3 me-3 d-flex align-items-center justify-content-center bg-${type === 'maestro' ? 'danger' : 'primary'}-subtle`}>
                    ...
                    <div className="flex-grow-1">
                      ...
                      <small className="text-secondary fw-bold" style={{ fontSize: '0.65rem' }}>REEMPLAZO TOTAL</small>
                    </div>
                  ...
                  <p className="mb-4 text-secondary" style={{ fontSize: '0.8rem' }}>
                    {type === 'maestro' ? 'Cargue el catálogo maestro de clientes y rutas.' : 'Actualice la demanda para sincronizar cuotas.'}
                  </p>
                  ...
                  <div className="p-3 mb-3" style={{ backgroundColor: 'var(--theme-background-tertiary)', border: '1px solid var(--theme-border-default)' }}>
                    ...
                    {metadataLoadingStatus[type] ? <GlobalSpinner variant={SPINNER_VARIANTS.IN_PAGE} /> : lastUploads[type] ? (
                      <Row className="g-2">
                        <Col xs={6}>
                          <small className="d-block text-secondary fw-bold text-uppercase" style={{ fontSize: '0.55rem' }}>Sincronización</small>
                          <span className="fw-black" style={{ fontSize: '0.75rem' }}>{lastUploads[type].updatedAt}</span>
                        </Col>
                        <Col xs={6}>
                          <small className="d-block text-secondary fw-bold text-uppercase" style={{ fontSize: '0.55rem' }}>Responsable</small>
                          <span className="fw-black d-block text-truncate" style={{ fontSize: '0.75rem' }}><FaUser size={10} /> {lastUploads[type].userName}</span>
                        </Col>
                        <Col xs={12}>
                          <div className="mt-1 fw-black text-success small">
                            <FaCheckCircle className="me-1" /> {lastUploads[type].rowCount.toLocaleString()} REGISTROS
                          </div>
                        </Col>
                      </Row>
                    ) : <small className="text-muted">Sin registros activos</small>}
                  </div>
          ...
          <div className="mt-4 p-4 border-top" style={{ borderColor: 'var(--theme-border-default)', backgroundColor: 'var(--theme-background-secondary)' }}>
            <div className="d-flex align-items-center mb-4 gap-2">
              <FaSpinner className={`${processingReports || isUploading ? 'spinner-animation' : ''} text-primary`} />
              <label className="small fw-black text-uppercase" style={{ color: 'var(--theme-text-primary)' }}>Inteligencia Logística y Reportes Operativos</label>
            </div>

            {isUploading && (
              <div className="mb-4">
                <ProgressBar now={uploadProgress} variant="danger" style={{ height: '4px' }} />
                <small className="fw-black text-danger mt-1 d-block text-end">SUBIENDO: {uploadProgress}%</small>
              </div>
            )}

            <Row className="g-3">
              {[
                { id: 'volumen', label: 'Ventas y Volumen', variant: 'success', icon: <FaShoppingCart /> },
                ...
                <Col key={rep.id} xs={12} md={6} lg={3}>
                  <div className="p-3 border h-100" style={{ background: 'var(--theme-background-primary)', borderColor: 'var(--theme-border-default)' }}>
      ...
      <style>{`
        ...
        .admin-section-table { background-color: var(--theme-background-primary) !important; }
